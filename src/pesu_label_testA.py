...
f1 = open("../data/submit.json", 'r') 
f2 = open("../data/preliminary_testA.txt", 'r', encoding='utf-8')
f3 = open("../data/train/preliminary_testA.txt.pesu_label", 'w', encoding='utf-8')


for line1, line2 in zip(f1.readlines(), f2.readlines()):
    line1, line2 = json.loads(line1), json.loads(line2)
    title = line2['title']
    if line1['match']['图文'] > 0.05 and  line1['match']['图文'] < 0.95:
        continue
    key_attr = {}
    match_ = {}
    for key in line1['match']:
        if key != '图文':
            if line1['match'][key] > 0.05 and line1['match'][key] < 0.95:
                continue
            if line1['match'][key] <= 0.05:
                match_[key] = 0
            if line1['match'][key] >= 0.95:
                match_[key] = 1
            for k in valsMap[tasksMap[key]].keys():
                if k in title:
                    key_attr[key] = k
                    break
                    
    if line1['match']['图文'] >= 0.95:
        match_['图文'] = 1
    if line1['match']['图文'] <= 0.05:
        match_['图文'] = 0
            
    line2.pop('query')
    line2['key_attr'] = key_attr
    line2['match'] = match_
    
    f3.write(json.dumps(line2, ensure_ascii=False)+'\n')
